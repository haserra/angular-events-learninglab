...
// global objects is never a good idea
// declare let toastr: { success: (arg0: any) => void; };
// so we should create a service that we will use to innject wherever required

@Component({
  selector: "events-list",
  //template: ``,
  templateUrl: "./events-list.component.html" /* notice this url is relative to this component */
})
export class EventsListComponent implements OnInit {
  ...
  events: any[]
  // workaround!!! 
  // should be: private eventService

  // workaround!!! 
  // should be: private toastr
  constructor(@Inject(EventService) eventService: EventService, @Inject(ToastrService) toastr: ToastrService) {
  ...
  /**
   * but we shouldn't call the service in the constructor (...)so let's use ngOnInit instead
   */
  ...
  /**
   * lets provide some sample data , eventually will ne provided from an API
   */
  /** this is a field (not a property ... )*/

  /** we have moved this array (field) out of this component to the Event Service */
